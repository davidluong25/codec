...
// Stub: In web mode, worktree setup failures are logged (WebSocket events will be added later)
function sendWorktreeSetupFailure(
  _windowId: number | null,
  payload: WorktreeSetupFailurePayload,
): void {
  console.warn("[worktree:setup-failed]", payload)
}
  ...
  generateCommitMessage: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!hasInternet) {
        ...
        try {
          const token: string | null = null // TODO: implement web auth
