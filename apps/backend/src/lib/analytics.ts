/**
 * PostHog analytics for 1Code Backend
...
const POSTHOG_DESKTOP_KEY = process.env.POSTHOG_KEY || "phc_wM7gbrJhOLTvynyhnhPkrVGDc5mKRSXsLGQHqM3T3vq"
...
function getFirstLaunchMarkerPath(): string {
  try {
    ...
    return path.join(dataDir, FIRST_LAUNCH_MARKER)
  } catch {
    return ""
  }
}
...
function getCommonProperties() {
  return {
    source: "backend",
    app_version: process.env.APP_VERSION || "0.0.58",
